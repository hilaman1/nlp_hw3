...
if args.variant == 'vanilla':
    # as model was imported to access GPT we need to model.GPT
    # as required to move the device to device defined on line 34.
...
if args.function == 'pretrain':
    ...
    if args.reading_params_path is not None:
        # For part c
        # loads model's parameters for inference
    ...
    """ 2. Finetune the model on this corpus """
    #Similarly to openning the pretrained text we need to open the corpus of finetune step in order to train our model on it.
    corpus_of_finetune = open(args.finetune_corpus_path, encoding='utf-8').read()

    dataset_finetune = dataset.NameDataset(pretrain_dataset, corpus_of_finetune)
    trainer.Trainer(model, dataset_finetune, None, tconf).train()
